...
Without this maximum value the value might overflow and instead be negative with high absolute value.
...
Here the values are not in the interval $[800,1200]$ but rather between 0 and the 9000.
...
This means at first the three \RLSR~variants from 2 to 3 to 4 and then the same for the \RLSN~variants.
...
With decreasing values for $\beta$ the amounts of flipped bits per step increases.
...
$pmut_{3.25}$ performs almost equally good with only the (1+1) EA variant performing clearly worse.
...
It seems many of the algorithms especially the variants of the RLS seem to be likely to get stuck in a local optimum.
...
They definitely are harder to solve for smaller input sizes as the binomial inputs, but they still have a perfect partition most times.
