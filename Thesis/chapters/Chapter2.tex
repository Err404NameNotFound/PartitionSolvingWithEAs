...
\begin{proof}
    ...
    The (1+1) EA has to minimise a linear function of $n-1$ bits which takes $\Theta(n\log{}n)$ time as well (Corollary 4.2 in~\cite{witt2013tight}).
    ...
    \begin{gather}
        ...
        \nonumber \le (1+\sqrt{6\ln(n)}) \cdot \frac{{(1+\sqrt{6\ln(n)})}^{3}}{n(n-1)}
        = \frac{{(1+\sqrt{6\ln(n)})}^{4}}{n(n-1)}\\ \nonumber
        = \frac{o({({n}^{1/40})}^{4})}{n(n-1)}
    ...
    The total runtime for the (1+1) EA is $(2en\ln(n)+4) + \Theta(n\log{}n) = \Theta(n\log{}n)$.
\end{proof}
