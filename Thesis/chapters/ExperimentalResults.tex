...
That's why for $m \in \{10,100,1000\}$ there is one figure for the even input sizes $n$ and one for the odd. 
For $n\ge10,000$ all values for the odd input sizes are 0 \%, so there is no point in showing the data in a separate graph. The lines in the separate graphs for even and odd $n$ are only to visualise the growth in $n$. The displayed values in between the listed input sizes on the $x$-axis do not have a meaning.\newline \TODO{rephrase this sentence to make it more obvious what is meant}
