...
As the greedy approach has nothing to do with Evolutionary Algorithms the proof is left an exercise for the reader.
A brief sketch here:\newline
...
For $y=1$ and $y=2$ there is an odd amount of odd values, so one bin must be even while the other has a difference of one, hence the solution is optimal in those cases as well.
...
The same lemma should be possible for the \RLSN[k] too.
Instead of looking at steps flipping only one bit here steps flipping $k$ bits are relevant as those happen with probability $\Theta(1)$.
The probability of choosing 4 bits from 0.3n is constant as well \(\binom{\lfloor 0.3n\rfloor}{k}/\binom{n}{k}=\frac{\Theta((0.3n)^k)}{\Theta(n^k)}=\Theta(0.3^k)=\Theta(1)\) for constant $k$.
This results in expected time $\bigO(n)$ for the \RLSN[k] to reach a search point with a $1+\frac{2k}{n+1}$-approximation.
